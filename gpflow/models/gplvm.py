...
class BayesianGPLVM(GPModel):
    ...
    def _build_likelihood(self):
        ...
        Kuu = self.kern.K(self.Z) + tf.eye(num_inducing, dtype=settings.np_float) * settings.numerics.jitter_level
        L = tf.cholesky(Kuu)
        ...
        NQ = tf.cast(tf.size(self.X_mean), settings.np_float)
        D = tf.cast(tf.shape(self.Y)[1], settings.np_float)
        ...
        ND = tf.cast(tf.size(self.Y), settings.np_float)
