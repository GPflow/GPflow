...
class SGPRUpperMixin(object):
    ...
    def compute_upper_bound(self):
        num_data = tf.cast(tf.shape(self.Y)[0], settings.np_float)
...
class SGPR(GPModel, SGPRUpperMixin):
    ...
    def _build_likelihood(self):
        ...
        num_data = tf.cast(tf.shape(self.Y)[0], settings.np_float)
        output_dim = tf.cast(tf.shape(self.Y)[1], settings.np_float)
        ...
        B = AAT + tf.eye(num_inducing, dtype=settings.np_float)
        LB = tf.cholesky(B)
    ...
    def _build_predict(self, Xnew, full_cov=False):
        ...
        L = tf.cholesky(Kuu)
        A = tf.matrix_triangular_solve(L, Kuf, lower=True) / sigma
        B = tf.matmul(A, A, transpose_b=True) + tf.eye(num_inducing, dtype=settings.np_float)
        LB = tf.cholesky(B)
        Aerr = tf.matmul(A, err)
        c = tf.matrix_triangular_solve(LB, Aerr, lower=True) / sigma
...
class GPRFITC(GPModel, SGPRUpperMixin):
    ...
    def _build_common_terms(self):
        ...
        B = tf.eye(num_inducing, dtype=settings.np_float) + tf.matmul(V / nu, V, transpose_b=True)
        L = tf.cholesky(B)
    ...
    def _build_likelihood(self):
        ...
        constantTerm = -0.5 * self.num_data * tf.log(tf.constant(2. * np.pi, settings.np_float))
