...
from ..util import create_logger, default_float, default_jitter, jitter_eye
...
def _conditional(Xnew, feat, kern, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
    ...
    Kmm = Kuu(feat, kern)  # M x M
    Kmn = Kuf(feat, kern, Xnew)  # M x N
    Knn = kern(Xnew, diag=(not full_cov))
    fmean, fvar = base_conditional(Kmn, Kmm, Knn, f, full_cov=full_cov, q_sqrt=q_sqrt, white=white)  # [N, R],  [R, N, N] or [N, R]
...
def _conditional(Xnew, X, kern, f, *, full_cov=False, q_sqrt=None, white=False):
    ...
    Kmm = kern(X) + jitter_eye(num_data)
    Kmn = kern(X, Xnew)
    Knn = kern(Xnew, diag=(not full_cov))
...
def base_conditional(Kmn, Kmm, Knn, f, *, full_cov=False, q_sqrt=None, white=False):
    ...
    if q_sqrt is not None:
        ...
        if full_cov:
            ...
            fvar = fvar + tf.reduce_sum(LTA ** 2, 1)  # R x N
...
def uncertain_conditional(Xnew_mu, Xnew_var, feat, kern, q_mu, q_sqrt, *,
    ...
    Kuu = feat.Kuu(kern, jitter=default_jitter())  # M x M
