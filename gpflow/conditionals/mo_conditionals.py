...
from ..features import (InducingPoints, MixedKernelSharedMof,
                        MixedKernelSeparateMof, SeparateIndependentMof,
...
@conditional.register(object, SharedIndependentMof, SharedIndependentMok,
                      object)
...
@conditional.register(object, SeparateIndependentMof, SeparateIndependentMok,
                      object)
@conditional.register(object, SharedIndependentMof, SeparateIndependentMok,
                      object)
@conditional.register(object, SeparateIndependentMof, SharedIndependentMok,
                      object)
def _conditional(Xnew,
                 feature,
                 kernel,
    ...
    logger.debug(
        "conditional: object, SharedIndependentMof, SeparateIndependentMok, object"
    )

    # Following are: [P, M, M]  -  [P, M, N]  -  [P, N](x N)
    Kmms = covariances.Kuu(feature, kernel,
                           jitter=default_jitter())  # [P, M, M]
    Kmns = covariances.Kuf(feature, kernel, Xnew)  # [P, M, N]
    kernels = kernel.kernels if isinstance(
        kernel, Combination) else [kernel.kernel] * len(feature.features)
    Knns = tf.stack(
...
def _conditional(Xnew,
    ...
    Kmm = covariances.Kuu(feature, kernel,
...
def _conditional(Xnew,
    ...
    if full_cov == full_output_cov:
        ...
        Knn = tf.reshape(Knn,
                         (N * K,
                          N * K)) if full_cov else tf.reshape(Knn, (N * K, ))
        fmean, fvar = base_conditional(Kmn,
                                       Kmm,
                                       Knn,
                                       f,
...
@conditional.register(object, (MixedKernelSharedMof, MixedKernelSeparateMof),
                      SeparateMixedMok, object)
def _conditional(Xnew,
                 feature,
                 kernel,
    ...
    logger.debug(
        "conditional: (MixedKernelSharedMof, MixedKernelSeparateMof), SeparateMixedMok"
    )
