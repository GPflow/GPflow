...
def sample_mvn(mean, cov, cov_structure):
    ...
    if cov_structure == "diag":
        ...
        cov = cov + (tf.eye(mean.shape[1], dtype=mean.dtype) * default_jitter())[None, ...]  # [N, P, P]
        chol = tf.linalg.cholesky(cov)  # [N, P, P]
...
def expand_independent_outputs(fvar, full_cov, full_output_cov):
    ...
    :param fvar: has shape N x P (full_cov = False) or [P, N, N] (full_cov = True).
    :return:
...
def independent_interdomain_conditional(Kmn, Kmm, Knn, f, *, full_cov=False, full_output_cov=False,
    ...
    :param Kmn: [M, L, N, P]
    :param Kmm: [L, M, M]
    :param Knn: N x P  or  N x N  or  [P, N, N]  or  [N, P, N, P]
    :param f: data matrix, M x L
    :param q_sqrt: [L, M, M]  or  M x L
    ...
    if full_cov and full_output_cov:
        ...
        At = tf.reshape(tf.transpose(Ar), (P, N, M * L))  # [P, N, M]L
        fvar = Knn - tf.matmul(At, At, transpose_b=True)  # [P, N, N]
    elif not full_cov and full_output_cov:
        At = tf.reshape(tf.transpose(Ar, [2, 3, 1, 0]), (N, P, M * L))  # [N, P, M]L
        fvar = Knn - tf.matmul(At, At, transpose_b=True)  # [N, P, P]
    elif not full_cov and not full_output_cov:
        fvar = Knn - tf.reshape(tf.reduce_sum(tf.square(A), [0, 1]), (N, P))  # Knn: N x P

    # another backsubstitution in the unwhitened case
    if not white:
        A = tf.linalg.triangular_solve(Lm, Ar)  # [L, M, M]  *  [L, M, N]P  ->  [L, M, N]P
        Ar = tf.reshape(A, (L, M, N, P))
    ...
    if q_sqrt is not None:
        ...
        if full_cov and full_output_cov:
            ...
            LTAr = tf.transpose(tf.reshape(LTA, (L * M, N, P)), [2, 0, 1])  # [P, LM, N]
            fvar = fvar + tf.matmul(LTAr, LTAr, transpose_a=True)  # [P, N, N]
        elif not full_cov and full_output_cov:
            LTAr = tf.transpose(tf.reshape(LTA, (L * M, N, P)), [1, 0, 2])  # [N, LM, P]
            fvar = fvar + tf.matmul(LTAr, LTAr, transpose_a=True)  # [N, P, P]
        elif not full_cov and not full_output_cov:
...
def fully_correlated_conditional(Kmn, Kmm, Knn, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
    ...
    :param Kmn: L[M, N, P]
    :param Kmm: LM x LM
    :param Knn: N x P or [N, P, N, P]
    :param f: data matrix, LM x 1
    :param q_sqrt: [1, LM, LM]  or 1 x ML
    :param full_cov: calculate covariance between inputs
    :param full_output_cov: calculate covariance between outputs
    :param white: use whitened representation
    :return:
...
def fully_correlated_conditional_repeat(Kmn, Kmm, Knn, f, *, full_cov=False, full_output_cov=False, q_sqrt=None,
    ...
    :param q_sqrt: [R, LM, LM]  or R x ML
    :param full_cov: calculate covariance between inputs
    :param full_output_cov: calculate covariance between outputs
    :param white: use whitened representation
    :return:
    ...
    if full_cov and full_output_cov:
        ...
        fvar = Knn - tf.matmul(At, At, transpose_b=True)  # [K, N, N]
    elif not full_cov and full_output_cov:
        # This transpose is annoying
        At = tf.transpose(Ar, [1, 0, 2])  # [N, M, K]
        # fvar = Knn - tf.einsum('mnk,mnl->nkl', Ar, Ar)
        fvar = Knn - tf.matmul(At, At, transpose_a=True)  # [N, K, K]
    elif not full_cov and not full_output_cov:
        # Knn: N x K
    ...
    if q_sqrt is not None:
        ...
        if full_cov and full_output_cov:
            ...
            addvar = tf.matmul(LTAr, LTAr, transpose_a=True)  # [R, K, N, N]
            fvar = fvar[None, ...] + addvar  # [R, K, N, N]
        elif not full_cov and full_output_cov:
            LTAr = tf.transpose(tf.reshape(LTA, (R, M, N, K)), [0, 2, 3, 1])  # [R, N, K, M]
            fvar = fvar[None, ...] + tf.matmul(LTAr, LTAr, transpose_b=True)  # [R, N, K, K]
        elif not full_cov and not full_output_cov:
