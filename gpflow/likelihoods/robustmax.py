...
class RobustMax(tf.Module):
    ...
    def __call__(self, F):
        i = tf.argmax(F, 1)
        return tf.one_hot(i, self.num_classes, tf.squeeze(1. - self.epsilon),
                          tf.squeeze(self.eps_k1))
    ...
    def prob_is_largest(self, Y, mu, var, gh_x, gh_w):
        ...
        oh_on = tf.cast(tf.one_hot(tf.reshape(Y, (-1, )), self.num_classes, 1.,
                                   0.),
                        dtype=mu.dtype)
        ...
        X = tf.reshape(mu_selected, (-1, 1)) + gh_x * tf.reshape(
            tf.sqrt(tf.clip_by_value(2. * var_selected, 1e-10, np.inf)),
            (-1, 1))
        ...
        oh_off = tf.cast(tf.one_hot(tf.reshape(Y, (-1, )), self.num_classes,
                                    0., 1.),
                         dtype=mu.dtype)
