...
@dispatch.expectation.register(Gaussian, (mfn.Linear, mfn.Constant), NoneType,
                               NoneType, NoneType)
...
@dispatch.expectation.register(Gaussian, mfn.Constant, NoneType, mfn.Constant,
                               NoneType)
...
@dispatch.expectation.register(Gaussian, mfn.MeanFunction, NoneType,
                               mfn.Constant, NoneType)
def _E(p, mean1, _, mean2, __, nghp=None):
    """
    Compute the expectation:
    expectation[n] = <m1(x_n)^T m2(x_n)>_p(x_n)
...
@dispatch.expectation.register(Gaussian, mfn.Identity, NoneType, mfn.Identity,
                               NoneType)
def _E(p, mean1, _, mean2, __, nghp=None):
    """
    Compute the expectation:
...
@dispatch.expectation.register(Gaussian, mfn.Identity, NoneType, mfn.Linear,
                               NoneType)
def _E(p, mean1, _, mean2, __, nghp=None):
    """
    Compute the expectation:
    ...
    e_xxt_A = tf.linalg.matmul(e_xxt, tf.tile(mean2.A[None, ...],
                                              (N, 1, 1)))  # NxDxQ
...
@dispatch.expectation.register(Gaussian, mfn.Linear, NoneType, mfn.Identity,
                               NoneType)
def _E(p, mean1, _, mean2, __, nghp=None):
    """
    Compute the expectation:
    ...
    N = tf.shape(p.mu)[0]
    e_xxt = p.cov + (p.mu[:, :, None] * p.mu[:, None, :])  # NxDxD
    e_A_xxt = tf.linalg.matmul(tf.tile(mean1.A[None, ...], (N, 1, 1)),
                               e_xxt,
...
@dispatch.expectation.register(Gaussian, mfn.Linear, NoneType, mfn.Linear,
                               NoneType)
def _E(p, mean1, _, mean2, __, nghp=None):
    """
    Compute the expectation:
    ...
    """
    e_xxt = p.cov + (p.mu[:, :, None] * p.mu[:, None, :])  # NxDxD
    e_A1t_xxt_A2 = tf.einsum("iq,nij,jz->nqz", mean1.A, e_xxt,
                             mean2.A)  # NxQ1xQ2
