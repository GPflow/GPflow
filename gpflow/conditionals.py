...
Option 1:
fmean : N x R         fvar : N x R  or  N x R x R  or  R x N x N  or  N x R x N x R
This is occurs when the multi-output nature is described purely by repeating the same prior R
times. Full covariances over outputs will always be diagonal.

Option 2:
fmean : N x P         fvar : N x P  or  N x P x P  or  P x N x N  or  N x P x N x P
For when we have a truly multi-output kernel, which we don't repeat R times.

Option 3:
fmean : R x N x P     fvar : R x N x P  or  R x N x P x P  or  R x P x N x N  or  R x N x P x N x P
Multi-output kernel, with R repetitions of the prior.
...
f      : M x R  or  M x L  or  R x M x L
q_sqrt : M x R  or  M x L  or  R x M x M  or L x M x M  or R x L x M x M  
"""
...
def conditional(Xnew, feat, kern, f, *, full_cov=False, full_cov_output=False, q_sqrt=None, white=False):
    ...
    if full_cov or full_cov_output:
