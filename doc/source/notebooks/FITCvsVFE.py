...
def plotPredictions( ax, model, color, label ):
    ...
    ax.plot( xtest, predMean, color, label=label , lw=2, alpha=1.0)
    ax.fill_between( xtest[:,0], predMean[:,0] - 2.*np.sqrt(predVar[:,0]), predMean[:,0] + 2.*np.sqrt(predVar[:,0]), color=color, alpha = 0.2)
