...
def find_modules(root: ModuleType) -> Sequence[ModuleType]:
    ...
    for path in root_path.glob("**/*.py"):
        ...
        if not hasattr(module, "__no_all__"):
            result.append(module)
    return result


_MODULES = find_modules(jn_gps)
_PACKAGES = [m for m in _MODULES if is_package(m)]
_MODULES_WITH_ALL = [m for m in _MODULES if hasattr(m, "__all__")]


@pytest.mark.parametrize("package", _PACKAGES)  # type: ignore[misc]
...
@pytest.mark.parametrize("module", _MODULES_WITH_ALL)  # type: ignore[misc]
...
def test_all_static(module: ModuleType) -> None:
    ...
    tree = ast.parse(module_path.read_text("utf-8"), str(module_path))
    ...
    assert all(
        isinstance(t, (ast.Constant, ast.Str))  # Actual type depends on Python version.
