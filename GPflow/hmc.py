...
def sample_HMC(f, num_samples, Lmax, epsilon, x0, verbose=False, thin=1, burn=0, RNG=np.random.RandomState(0)):
    """
    ...
    Lmax and epsilon are parameters of the hmc procedure to be tuned (more below).
    x0 is the starting position for the procedure
    verbose is a flag which turns on the display of the running accept ratio, etc.
    ...
    if burn > 0:
        ...
        samples = sample_HMC(f, num_samples=burn, Lmax=Lmax, epsilon=epsilon, x0=x0, verbose=verbose, thin=1, burn=0)
        if verbose:
    ...
    for t in range(1, num_samples * thin):
        ...
        if(((t+1) % 100) == 0):
            if verbose: print("Iteration: ", t+1, "\t Acc Rate: ", 1. * accept_count_batch, "%")
            accept_count_batch = 0
        ...
        #reject the proposal if there are numerical errors. Shouldn't happen too often...
