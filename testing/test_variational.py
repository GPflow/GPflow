...
class VariationalUnivariateTest(unittest.TestCase):
    ...
    def get_model(self, is_diagonal, is_whitened):
        ...
        if is_diagonal:
            m.q_sqrt = np.ones((self.univariate, self.oneLatentFunction))*self.posteriorStd
        else:
    ...
    def test_build_likelihood(self):
        ...
        log_marginal_likelihood = referenceUnivariateLogMarginalLikelihood(y=self.y_real,
                                                                           K=self.K, noiseVariance=self.noiseVariance)
...
class VariationalMultivariateTest(unittest.TestCase):
    ...
    def test_refrence_implementation_consistency(self):
        ...
        multivariate_KL = referenceMultivariatePriorKL(np.array([[qMean]]), np.array([[qCov]]),
