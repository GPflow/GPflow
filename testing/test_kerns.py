...
class TestRbf(ParallelTestCase):
    def test_1d(self):
        lengthScale = 1.4
        variance = 2.3
...
class TestArcCosine(ParallelTestCase):
...
class TestPeriodic(ParallelTestCase):
...
class TestCoregion(ParallelTestCase):
    def setUp(self):
...
class TestKernSymmetry(ParallelTestCase):
    def setUp(self):
...
class TestKernDiags(ParallelTestCase):
    def setUp(self):
        tf.reset_default_graph()
        inputdim = 3
        rng = np.random.RandomState(1)
...
class TestWhite(ParallelTestCase):
    """
...
class TestSlice(ParallelTestCase):
    """
...
class TestProd(ParallelTestCase):
    def setUp(self):
        tf.reset_default_graph()
...
class TestARDActiveProd(ParallelTestCase):
    def setUp(self):
        tf.reset_default_graph()
        self.rng = np.random.RandomState(0)
...
class TestKernNaming(ParallelTestCase):
...
class TestKernNamingProduct(ParallelTestCase):
    def test_no_nesting_1(self):
        k1 = GPflow.kernels.RBF(1)
        k2 = GPflow.kernels.Linear(2)
        k3 = k1 * k2
...
class TestARDInit(ParallelTestCase):
    """
