...
class TestMeanFuncs(GPflowTestCase):
    ...
    def mfs1(self):
        ...
        return [gpflow.mean_functions.Zero(),
                gpflow.mean_functions.Linear(
                    rng.randn(self.input_dim, self.output_dim).astype(default_float()),
                    rng.randn(self.output_dim).astype(default_float())),
    ...
    def test_basic_output_shape(self):
        with self.test_context() as sess:
            X = tf.placeholder(default_float(), shape=[self.N, self.input_dim])
            X_data = np.random.randn(self.N, self.input_dim).astype(default_float())
            for mf in self.mfs1():
                mf.compile()
    ...
    def test_add_output_shape(self):
        with self.test_context() as sess:
            X = tf.placeholder(default_float(), [self.N, self.input_dim])
            X_data = np.random.randn(self.N, self.input_dim).astype(default_float())
...
class TestModelCompositionOperations(GPflowTestCase):
    ...
    Xtest = rng.randn(30, 3).astype(default_float())
    ...
    def initials(self):
        ...
        linear1_1 = gpflow.mean_functions.Linear(
            self.rng.randn(self.input_dim, self.output_dim).astype(default_float()),
            self.rng.randn(self.output_dim).astype(default_float()))
        self.rng.seed(seed=0)
    ...
    def setUp(self):
        ...
        with self.test_context():
            ...
            X = self.rng.randn(self.N, self.input_dim).astype(default_float())
            Y = self.rng.randn(self.N, self.output_dim).astype(default_float())
...
class TestModelsWithMeanFuncs(GPflowTestCase):
    ...
    def setUp(self):
        ...
        rng = np.random.RandomState(0)
        X, Y, Z, self.Xtest = (
            rng.randn(self.N, self.input_dim).astype(default_float()),
            rng.randn(self.N, self.output_dim).astype(default_float()),
            rng.randn(self.M, self.input_dim).astype(default_float()),
            rng.randn(self.Ntest, self.input_dim).astype(default_float()))
