...
def test_shared_independent_mok_with_inducing_points():
    with session_context() as sess:
        ...
        mean1, var1 = predict(sess, m1, Data.Xs, full_cov=True, full_cov_output=True)  # Ns x P, Ns x P x Ns x P
        mean2, var2 = predict(sess, m2, Data.Xs, full_cov=True, full_cov_output=True)  # Ns x P, Ns x P x Ns x P
        ...
        mean3, var3 = predict(sess, m1, Data.Xs, full_cov=True, full_cov_output=False)  # Ns x P, P x Ns x Ns
        mean4, var4 = predict(sess, m2, Data.Xs, full_cov=True, full_cov_output=False)  # Ns x P, P x Ns x Ns
        ...
        mean5, var5 = predict(sess, m1, Data.Xs, full_cov=False, full_cov_output=True)  # Ns x P, Ns x P x P 
        mean6, var6 = predict(sess, m2, Data.Xs, full_cov=False, full_cov_output=True)  # Ns x P, Ns x P x P
        ...
        mean7, var7 = predict(sess, m1, Data.Xs, full_cov=False, full_cov_output=False)  # Ns x P, Ns x P
        mean8, var8 = predict(sess, m2, Data.Xs, full_cov=False, full_cov_output=False)  # Ns x P, Ns x P
...
# def test_shared_independent_mok_with_shared_independet_mof():
#     with session_context() as sess:
#         q_mu_1 = np.random.randn(Data.M * Data.P, 1)  # MP x 1
#         q_mu_2 = np.reshape(q_mu_1, [Data.M, Data.P])  # M x P
#         q_sqrt_2 = np.array([np.tril(np.random.randn(Data.M, Data.M)) for _ in range(Data.P)])  # P x M x M
