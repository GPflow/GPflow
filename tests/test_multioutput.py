...
def check_equality_predictions(sess, models, dec=3):
    ...
    assert_all_array_elements_almost_equal(all_means, decimal=dec)
    ...
    _ = [assert_all_array_elements_almost_equal(var, decimal=dec) for var in all_vars]
    ...
    np.testing.assert_almost_equal(np.diagonal(var_tt, axis1=1, axis2=3),
                                   np.transpose(var_tf, [1, 2, 0]), decimal=dec)
    np.testing.assert_almost_equal(np.diagonal(var_tt, axis1=0, axis2=2),
                                   np.transpose(var_ft, [1, 2, 0]), decimal=dec)
