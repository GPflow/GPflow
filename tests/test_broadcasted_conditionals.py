...
@pytest.mark.parametrize("conditional_type",
                         ["mixing", "Z", "inducing_points"])
def test_conditional_broadcasting(full_cov, white, conditional_type):
    ...
    if conditional_type == "Z":
        ...
        feature = mf.MixedKernelSharedMof(
            gpflow.features.InducingPoints(Data.Z))
        kernel = mk.SeparateMixedMok(kernels=[
            gpflow.kernels.Matern52(lengthscale=0.5) for _ in range(Data.L)
        ],
                                     W=Data.W)
    else:
    ...
    num_samples = 5
    sample_conditional_fn = lambda X: sample_conditional(
        X,
        feature,
        kernel,
    ...
    assert_allclose(samples_S1_S2.shape,
    ...
    if full_cov:
        ...
        vars_s1_s2 = vars_S1_S2.numpy().reshape(Data.S1 * Data.S2, Data.N,
                                                Data.Dy)
        assert_allclose(vars_s1_s2, variables)
...
def test_broadcasting_mix_latent_gps(full_cov, full_output_cov):
    ...
    f_mu, f_var = mix_latent_gp(tf.convert_to_tensor(W),
