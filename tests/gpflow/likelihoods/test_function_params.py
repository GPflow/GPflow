...
class Datum:
    ...
    X_positive = rng.random(X_shape)
    X_positive[:, :, :, 0] = np.linspace(0.1, 1.0, N)[None, None, :]
    X_negative = -rng.random(X_shape)
    F = 0.5 * np.ones(Y_shape)
    Fmu = 0.5 * np.ones(Y_shape)
    Fvar = 0.1 * np.ones(Y_shape)
    Y = 0.5 * np.ones(Y_shape)
