...
def test_scipy_jit():
    ...
    opt1.minimize(
        closure1, variables=m1.trainable_variables, options=dict(maxiter=50), compile=False
    )
    opt2.minimize(
        closure2, variables=m2.trainable_variables, options=dict(maxiter=50), compile=True
    )
