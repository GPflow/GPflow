...
def referenceRbfKernel(X, lengthscale, signal_var):
    N, _ = X.shape
    kernel = np.zeros((N, N))
    for row_index in range(N):
        for column_index in range(N):
            ...
            kernel[row_index, column_index] = signal_var * np.exp(-0.5 * distance_squared /
                                                                  lengthscale ** 2)
    return kernel


def referenceArcCosineKernel(X, order, weightVariances, biasVariance, signal_var):
...
def referencePeriodicKernel(X, lengthscale, signal_var, period):
